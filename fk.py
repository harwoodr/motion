...
#legMin = [1.7272]*6
#legMax = [2.1336]*6
#L = numpy.array([1.7272, 1.7272, 1.7272, 1.7272, 1.7272, 1.7272]).transpose()
L = numpy.array([2.1336, 2.1336, 2.1336, 2.1336, 2.1336, 2.1336]).transpose()
#L = numpy.array([2.1,1.5, 2.1, 1.5, 2.1, 1.5]).transpose()
a = fk(bPos, pPos, L)
print(a)
#print(ik(bPos, pPos, a))
#a = numpy.array([0,0,0, 1, 0, 0]).transpose()
#print(ik(bPos, pPos, a))
names = list(["x","y","z","Pitch?","Roll?","Yaw"])
for i in range(0,6):
    print(names[i],end=": ")
    if i<3:
        print(a[i])
    else:
        print(math.degrees(a[i]))
#Other test
'''
lengths = []
